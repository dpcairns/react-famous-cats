...
- [Pair and Mob Programing](#pair-mob-programing)
- [Standup](#standup)
...
Use post-it notes and a whiteboard to manage your project. Populate your board with tasks. Each task should be a small item, typically linked to a feature task. As you work on a task, add yourself as a member on the card and move it into the "In progress" column. This will provide an easy way for everyone on the team to see the progress of each task and know who is working on a particular task.
...
_Why:_
> Any items in a `.gitignore` file will not be pushed to GitHub. This is useful for files such as `.env` and `.DS_Store` and `node_modules`.
...
You are advised to follow the project structure we have been using in class.
...
<a id="pair-mob-programing"></a>
## Pair and Mob Programing
As a team, you can determine when and how to pair or mob program. Strive to be effective and match the work to the best strategy for your team.

**Pair and Mob Programming _must_ be done with a timer and disciplined role switching**

_Why:_
> Different working styles can help effectively complete different types of tasks and help team members learn from each other.

Pairing and mobbing can turn into people watching on person do work if the rules and roles are not applied
...
After the initial repository creation, scaffold out a basic `server.js` and other files. Then push this code to GitHub and deploy it as a "proof of life" that the deployment worked correctly. As you continue to work on your project, the deployed site will update any time the team merges a pull request into the master branch. You also have the option to set this to a different branch, although it is most common to deploy from the master branch.
...
In addition to the scheduled practice session, the team is encouraged to continue to practice on their own. Keep track of the time and adjust accordingly.
...
- Good and proper use of JavaScript and Components
  - Clear, readable, and efficient code
  - Uses asynchronous JavaScript to manage a distributed system correctly
  - Uses components to "compose" the UI and follows data-down/events-up
...
- Generally useful and functional
  - The project is encouraged to utilize a custom top-level domain name
